...
const { tnc } = require('./database/menu/tnc')
const { gun } = require('./database/menu/gun')
const { donate } = require('./database/menu/donate')
const { rules } = require('./database/menu/rules')
...
async function starts() {
	...
	fs.existsSync('./asw.json') && client.loadAuthInfo('./asw.json')
	...
	await client.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./asw.json', JSON.stringify(client.base64EncodedAuthInfo(), null, '\t'))
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["6281803390353@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '6281803390353@s.whatsapp.net'
                        const isEventon = isGroup ? event.includes(from) : false
                        const isRegister = checkRegisteredUser(sender)
                        pushname = client.contacts[sender] != undefined ? client.contacts[sender].vname || client.contacts[sender].notify : undefined

                        // Options
                        const botName = '*GUNTUR BOT*'
                        const ownerName = '*GUNTUR PRASETYO*'
			...
			switch(command) {
			                ...
			                const uangku = checkATMuser(sender)
                                ...
                                case 'limitmenu':
                                 ...
                                 case 'infobot':
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        await costum(gun(prefix, botName, ownerName), text, tescuk, cr)
                                        break 
                                case 'donate':
                                case 'iklan':
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        await costum(donate(prefix, botName, ownerName), text, tescuk, cr)
                                        break 
                                case 'rules':
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        await costum(rules(prefix, botName, ownerName), text, tescuk, cr)
                                        break 
                                case 'tnc':
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        await costum(tnc(prefix, botName, ownerName), text, tescuk, cr)
                                        break 
                                case 'info':
                                if (!isRegister) return reply(mess.only.daftarB)
					me = client.user
					uptime = process.uptime()
					teks = `*Nama bot* : ${botName}\n*Author* : ${ownerName}\n*Nomor Bot* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Total Block Contact* : ${blocked.length}\n*The bot is active on* : ${kyun(uptime)}\n*Total Chat* : ${totalchat.length}\n*Instagram* : *COMING SOON*\n*Github* : *COMING SOON*\n*Yt Chanel* : *COMING SOON*`
               ...
               break
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Perintah di terima, menghapus jabatan @${mentioned[0].split('@')[0]} sebagai admin`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                  case 'promote':
					...
					case 'odemote':
					if (!isGroup) return reply(mess.only.group)
				    if (!isOwner) return reply(mess.only.mess.only.ownerB)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('𝐓𝐚𝐠 𝐭𝐚𝐫𝐠𝐞𝐭 𝐲𝐚𝐧𝐠 𝐦𝐚𝐮 𝐝𝐢 𝐭𝐮𝐫𝐮𝐧𝐤𝐚𝐧 𝐚𝐝𝐦𝐢𝐧')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
  ...
  client.updatePresence(from, Presence.composing) 
	...
	case 'hidetag':
                ...
                if (!isRegister) return reply(mess.only.daftarB)
                if (!isGroup) return reply(mess.only.group)
                if (!isGroupAdmins) return reply(mess.only.admim)
                var value = body.slice(9)
					var group = await client.groupMetadata(from)
					var member = group['participants']
					var mem = []
               ...
               if (data.error) return reply(data.error)
				...
				case 'blocklist':
                   ...
                   case 'chatlist':
					...
					for (let register of register) {
						teks += `~> @${register.split('@')[0]}\n`
					}
					teks += `Total : ${register.length}`
					client.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": register}})
					break
				case 'animecry':
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 GUNTUR BROADCAST 」*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*「 GUNTUR BROADCAST 」*\n\n${body.slice(4)}`)
						}
	    ...
	    case 'kapankah':
	          ...
	          if (!isRegister) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
                ...
                case 'truth':
					...
					client.sendMessage(from, tod, image, { caption: '*Dare*\n\n'+ der, quoted: mek })
					break	
					case 'slot':
					if (!isRegister) return reply(mess.only.daftarB)
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					const slot =['🍏','🍎','🍐','🍊','🍋','🍌','🍉','🍇','🥝','🥥','🍍','🥭','🍑','🍒','🍈','🍓','🍅','🍆','🥑','🥦','🥬','🥒','🌶️','🌽','🥕','🧄','🧅','🥔','🍠']
					const sl1 = slot[Math.floor(Math.random() * slot.length )]
					const sl2 = slot[Math.floor(Math.random() * slot.length )]
					const sl3 = slot[Math.floor(Math.random() * slot.length )]
					const sl4 = slot[Math.floor(Math.random() * slot.length )]
					const sl5 = slot[Math.floor(Math.random() * slot.length )]
					const sl6 = slot[Math.floor(Math.random() * slot.length )]
					const sl7 = slot[Math.floor(Math.random() * slot.length )]
					const sl8 = slot[Math.floor(Math.random() * slot.length )]
					const sl9 = slot[Math.floor(Math.random() * slot.length )]
					//sloting = '[  🎰 | *SLOTS* | 🎰 ]\n -------------------------------------------------\n ❌❌❌ '+sl+' : '+sl+' : '+sl+' ❌❌❌\n *=======>* '+sl+' : '+sl+' : '+sl+' *<=======*\n ❌❌❌ '+sl+' : '+sl+' : '+sl+' ❌❌❌\n -------------------------------------------------\n[  🎰 | *SLOTS* | 🎰 ]\n\n Keterangan : *Jika anda mendapatkan 1 slot dibagian tengah yg berisi 3 buah sama, berarti anda adalah pemenang games slot*\n\n *Contoh : 🍌 : 🍌 : 🍌<=====*'
					slotgamesanj = await getBuffer(`https://i.ibb.co/T0dQDSb/slotgamesanj.jpg`)
					client.sendMessage(from, slotgamesanj, image, { quoted: mek, caption: '》 🎰 | *SLOTS* | 🎰 《\n -------------------------------------------------\n ❌❌❌ '+sl1+' : '+sl2+' : '+sl3+' ❌❌❌\n *=======>* '+sl4+' : '+sl5+' : '+sl6+' *<=======*\n ❌❌❌ '+sl7+' : '+sl8+' : '+sl9+' ❌❌❌\n -------------------------------------------------\n》 🎰 | *SLOTS* | 🎰 《\n\n Keterangan : \n*1. Jika anda mendapatkan 1 slot dibagian tengah yg berisi 3 buah sama, berarti anda adalah pemenang games slot*\n\n *Contoh : 🍌 : 🍌 : 🍌<=====*'})
					await limitAdd(sender)
					break	
                case 'level':
            ...
            break
                                ...
                                case 'neonlogo2':
                                        ...
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/textpro?theme=neon_technology&text=${teks1}&apikey=BotWeA`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih kak logonya...'})
                                        await limitAdd(sender)
                                        break
